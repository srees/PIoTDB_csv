...
# ##########################################
def main():
    # Initialize a couple variables that may not otherwise be set
    date = ''
    topic = ''
    ...
    alignment = input("Enter 'A' for aligned data or 'U' (default) for unaligned data: ")
    align = False
    if alignment == 'A':
        align = True
    if not align:
    ...
    field_types = input(
        "Enter a space-separated list of measurement types corresponding to the measurement names: ").split()
    ...
    verify_count = insert_can_data(filepath, experiment, date, topic, field_names, field_types, total_lines, align,
                                   batch_size)
...
def insert_can_data(filepath, experiment, date, topic, field_names, field_types, total_lines, align, batch_size=100):
    ...
    with open(filepath, 'r') as csvfile:
        ...
        for row in csvreader:
            ...
            if align:
                # Case for aligned data

                # In our use case, GPS data is the only aligned data, and our CSV combines all dates
                # I think, technically, we could have done this for the unaligned canbus data as well...
                date = f"2022_11_{row[-1].split('_')[-2]}"
                ...
                base_path = f"root.{experiment}.vehicle_{vehicle_num}.{date}.run_{run_number}.gps_data"

                # Ignoring num_fields because we are inserting them all at once as aligned data, so it won't alter the insert count
                num_fields = 1

                # Create time series if it doesn't exist
                # canbus data was in files sorted by topic. There is no topic for gps_data.
                time_series_path = base_path
                if time_series_path not in created_ts:
                    create_aligned_time_series(session, time_series_path, field_names, field_types)
                ...
                measurements.append(field_names)
                if not verify_measurement:
                    verify_measurement = field_names[0]

                # Get and convert the values for this row: we only care about columns 0-7
                value_lst = [convert_value(column, field_types[i]) for i, column in enumerate(row[:8])]
                # Add the converted value to the batch
                values.append(value_lst)

                # Add measurement field type data
                map_types = [map_field_type(i) for i in field_types]
                d_types.append(map_types)
            ...
            # Check if batch size is reached and insert data if so.
            # This could be over batch_size due to the loop for fields, but that should still be fine.
            if len(paths) >= batch_size:
                ...
                # Insert the data
                if align:
                    session.insert_aligned_records(paths, timestamps, measurements, d_types, values)
                else:
...
def create_aligned_time_series(session, time_series_path, measurements, field_types):
    ...
    if not session.check_time_series_exists(check_path):
        ...
        compression_list = [Compressor.SNAPPY] * len(field_types)
        session.create_aligned_time_series(time_series_path, measurements, iotdb_types, enc_list, compression_list)
        print(
            f"Time series {time_series_path} created with {measurements} of {iotdb_types} type, PLAIN encoding, and SNAPPY compression.")
